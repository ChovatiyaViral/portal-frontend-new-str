...
export function getKeyValuePair(data, key, isReversed = true, otherKey) {
  ...
  return inputData.map((list) => {
    if (key) {
      if (otherKey) {
        return { label: get(list, otherKey, ""), value: get(list, key, "") };
      } else {
...
export const setAppLoader = (value) => {
  ...
  if (value) {
    document.getElementById("app_loading").classList.remove("app_pageLayout__loading_inactive");
    document.getElementById("app_loading").classList.add("app_pageLayout__loading_active");
    document.body.style.overflow = "hidden";
  } else {
    document.getElementById("app_loading").classList.remove("app_pageLayout__loading_active");
    document.getElementById("app_loading").classList.add("app_pageLayout__loading_inactive");
    document.body.style.overflow = "visible";
  }
};

export const setAppSuccessUI = (value) => {
  window.scroll({ top: 0, behavior: "smooth" });
  if (value) {
    document.getElementById("app_success_ui").classList.remove("app__success_state_inactive");
    document.getElementById("app_success_ui").classList.add("app__success_state_active");
    document.body.style.overflow = "hidden";
  } else {
    document.getElementById("app_success_ui").classList.remove("app__success_state_active");
    document.getElementById("app_success_ui").classList.add("app__success_state_inactive");
    document.body.style.overflow = "visible";
  }
};

export const getDiscrepancyColor = (value) => {
...
export const scrollToDiv = (ele) => {
  window.scrollTo(0, findPosition(document.getElementById(ele)));
};
...
export const getBlobURL = (s3URL, binaryData) => {
  const fileExt = s3URL.split(".").pop();
  const link = `${getFileType(fileExt)};base64,${binaryData}`;
  const blob = base64toBlob(binaryData, getFileType(fileExt));
