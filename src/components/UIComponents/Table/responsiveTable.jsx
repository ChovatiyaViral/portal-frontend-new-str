import { DownloadOutlined, FilterOutlined, PlusOutlined, SearchOutlined, UndoOutlined,ReloadOutlined } from "@ant-design/icons";
...
class CustomResponsiveTable extends React.Component {
  ...
  getColumnSearchProps = (dataIndex, filters, title) => ({
    ...
    filterDropdownOpen: get(this, "props.isFilterable", false),
    filteredValue: get(this, `state.filteredInfo.${dataIndex}`, null) || null,
    filterIcon: (filtered) => {
      return get(this, "props.isFilterable", false) && <FilterOutlined style={{ color: filtered ? "#1890ff" : undefined }} onClick={(e) => e.preventDefault()} />;
    },
  ...
  getColumnsBasedOnType = () => {
    ...
    if (columnType === TableColumnsList.CRRList) {
      return {
        scroll: { x: 1500 },
        columns: getColumns,
      };
    ...
    if (columnType === TableColumnsList.SamplingHistory) {
      return {
        scroll: { x: 871 },
        columns: getColumns,
      };
    ...
    if (columnType === TableColumnsList.SpiritTransfer) {
      return {
        scroll: { x: 970 },
        columns: getColumns,
      };
    ...
    if (columnType === TableColumnsList.CaskMasterList) {
      return {
        scroll: { x: 2500 },
        columns: getColumns,
      };
    ...
    if (columnType === TableColumnsList.PassportMasterList) {
      return {
        scroll: { x: 1100 },
        columns: getColumns,
      };
  ...
  getStyle = () => {
    if (get(this, "props.style")) {
      return get(this, "props.style");
    }
    if (getScreenSize() > isMobileOrTab) {
      return { top: "-4px" };
    } else {
      return { top: "-40px", marginBottom: 50 };
    }
  };
  ...
  render() {
    ...
    return (
      <>
        <div style={this.getStyle()} className="search_filter_position">
          {get(this, "props.isExportAvailable", true) && get(this, "props.data", []).length > 0 && (
          ...
          {get(this, "props.isGlobalFilterEnabled", false) && get(this, "props.data", []).length > 0 && (
        ...
        {getScreenSize() > isMobileOrTab ? (
          <div className="position-relative">
            <Table
              columns={columns}
              tableLayout={get(this, "props.tableLayout", "fixed")}
