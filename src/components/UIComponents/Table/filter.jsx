import { InfoCircleOutlined, CloseCircleOutlined, ClearOutlined, CheckOutlined } from "@ant-design/icons";
...
const CustomFilter = props => {
  ...
  const getItems = () => {
    return filterColumns.map((list, index) => {
      ...
      return {
        key: list,
        label: <Badge dot={dotValue}>{getTitle(list)}</Badge>,
        children: (
          <>
            {!dateTimeType && (
              <div style={{ width: "90%" }}>
    ...
    <>
      <Modal
        ...
        footer={[
          ...
          <Button key="back" onClick={() => props.onClose()} icon={<CloseCircleOutlined />}>
            Cancel
          </Button>,
      ...
      >
        <Tabs tabPosition="top" activeKey={currentTab} className="mobile_card__filter__tabs" onChange={onTabChange} items={getItems()} />
      </Modal>
    </>
  );
};
