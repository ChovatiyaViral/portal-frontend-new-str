...
import { cloneDeep, find, get, round } from "lodash";
...
const EditRotationNumber = (props) => {
  ...
  const save = async (key) => {
    try {
      ...
      const available_qty = Number(get(row, "whole_case", 0)) + BIPC;
      ...
      if (sumValue > 0) {
        ...
        if (index > -1) {
          newData.splice(index, 1, { ...item, ...row, available_qty, allocated_qty: get(item, "allocated_qty", 0) });
          setDataSource(newData);
          setEditingKey("");
        } else {
          newData.push(row);
    ...
    if (get(rest, "data.status")) {
      let newData = get(rest, "data.rotation_numbers", []).map((list, index) => {
        return {
          key: index,
          whole_case: get(list, "whole_case", 0),
          bottles_in_partial_case: get(list, "bottles_in_partial_case") ? `${get(list, "bottles_in_partial_case", 0)}/${get(props, "record.bpc", 0)}` : undefined,
          rotation_number: get(list, "rotation_number", ""),
          available_qty: get(list, "available_qty", 0),
          allocated_qty: get(list, "allocated_qty", 0),
        };
      });
