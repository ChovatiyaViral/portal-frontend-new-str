...
import { capitalizeAllLetter, getFileType, getScreenSize, numberWithCommas } from "../../../../helpers/utility";
          ...
          <div className="CFA">
            ...
            <Tooltip placement="topLeft" title="Download Order Details">
              <Button type="primary" className="ml-sm-3" disabled={!get(permissions, "download_order_details")} icon={<DownloadOutlined />} size="small" onClick={() => downloadSalesDetails()}>
                {getScreenSize() > isMobileOrTab && "Download Order Details"}
              </Button>
            </Tooltip>
            <Tooltip placement="topLeft" title=" View Proforma Sales Doc">
              <Button type="primary" disabled={!get(permissions, "view_proforma_sales_doc")} className="ml-2 ml-sm-3" icon={<EyeOutlined />} size="small" onClick={() => setProformaModalIsOpen(true)}>
                {getScreenSize() > isMobileOrTab && "View Proforma Sales Doc"}
              </Button>
            </Tooltip>
            <Tooltip placement="topLeft" title="Convert to Sales Order">
              <Button type="primary" className="ml-2" disabled={!get(permissions, "convert_to_sales_order")} icon={<SwapOutlined />} size="small" onClick={() => info()}>
                {getScreenSize() > isMobileOrTab && `Convert to ${getOrderTypeForConversions(get(props, "sales_order_type", "NA"))}`}
              </Button>
            </Tooltip>
          </div>
              ...
              <Col>
                <Row style={{ fontSize: "0.9rem" }} gutter={[16, 16]}>
                  ...
                  <Col lg={{ span: 15 }}>
                    <Text type="secondary">
                      Target Region:
                      {get(permissions, "update_target_region") && <EditOutlined className="pl-2" onClick={() => setTargetRegionModalIsOpen(true)} />}
                    </Text>
