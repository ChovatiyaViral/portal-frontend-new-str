import { EditOutlined, FilePdfOutlined, FolderAddOutlined, SaveOutlined } from "@ant-design/icons";
    ...
    <>
      <ErrorBoundary>
        <Spin spinning={get(props, "loading", false)}>
          ...
          <div className="common_card_section pl-3 pr-3 pb-0">
            ...
            <Row style={{ fontSize: "0.9rem" }}>
              <Col xs={{ span: 24 }}>
                <div className="d-flex justify-content-between">
                  ...
                  {get(permissions, "update_documents") && get(props, "documents", []).length > 0 && (
                    <Button icon={<EditOutlined />} type="primary" onClick={() => setUpdateDocumentsIsOpen(true)}>
                      {getScreenSize() > isMobileOrTab && "Edit"}
                    </Button>
                  )}
                </div>
                <hr style={{ backgroundColor: "#ccd5df", margin: "5px 0 20px" }} />
              ...
              {get(props, "documents", []).map((doc, index) => {
                return (
                  <Col sm={{ span: 8 }} key={index} style={{ cursor: "pointer" }} className="text-overflow mb-3">
