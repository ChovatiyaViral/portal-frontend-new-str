...
import { isMobileOrTab, MasterDataKeyPair } from "../../../../constants";
import { capitalizeAllLetter, decimalWithTwoPointsCheck, getKeyValuePair, getScreenSize, isBlank, numberWithCommas } from "../../../../helpers/utility";
...
const AddSpiritModal = (props) => {
    ...
    return [
      {
        key: "spirit",
        label: "Add Spirit",
        children: <div className="add-spirit-modal w-100">
          <Row gutter={[16, 0]}>
      ...
      },
      {
        key: "bundle",
        label: "Add Bundle",
        children: <Spin spinning={bundleListLoading}>
          <div className="site-card-wrapper mb-4">
            <Row gutter={[16, 16]}>
              {bundleList.map((list, index) => {
                return (
                  <Col sm={{ span: 8 }} key={index}>
          ...
          <div className="mt-4 mb-0" style={{ width: `${getScreenSize() > isMobileOrTab ? "30%" : "100%"}` }}>
            <InputNumberChange
  ...
  return (
    ...
    >
      <>
        <Tabs defaultActiveKey="spirit" value={currentTab} onChange={onChange} items={getItems()} />
      </>
    </Modal>
  );
};
