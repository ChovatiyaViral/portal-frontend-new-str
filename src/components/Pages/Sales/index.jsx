...
const SalesOrder = (props) => {
  ...
  const [orderPlacementLoader, setOrderPlacementLoader] = React.useState(false);

  React.useEffect(() => {
    dispatch(setCurrentView(`Creating New - ${capitalizeAllLetter(salesOrderType.replace(/_/g, " "))}`));
  }, [salesOrderType]);
  ...
  const handleNext = () => {
    ...
    if (get(props, "inputJSON.status", false)) {
      if (currentStep === 0) {
        ...
        const valid = validCheck(customerDetailObj, validatorCustomerDetailsJSON);
        if (valid) {
          if (get(customerDetailObj, "country") === "United Kingdom" && !get(customerDetailObj, "vat_no")) {
            openNotificationWithIcon("error", `Order Type - ${capitalizeAllLetter(salesOrderType.replace(/_/g, " "))}`, "VAT No. is mandatory for United Kingdom.");
          } else {
      ...
      <>
        <div className="new-sales-order">
          ...
          <OrderConfirmation
            ...
            orderPlacementLoader={orderPlacementLoader}
            handleOrderConfirmationModal={handleOrderConfirmationModal}
            handleConfirmationSubmit={() => {
              setOrderPlacementLoader(true);
              handleFinalSubmit();
            }}
          />
          {viewDocumentStatus && (
